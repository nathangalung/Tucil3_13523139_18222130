...
public class BoardPanel extends JPanel {
    ...
    private void drawExit(Graphics2D g2d, int rows, int cols, FontMetrics fm) {
        ...
        if (exitR == -1 && exitC == -1 && board.getPrimaryPiece() == null) return;
        ...
        if (exitC == cols) { kRectX = cols * cellSize; kRectY = exitR * cellSize; }
        else if (exitC == -1) { kRectX = -cellSize; kRectY = exitR * cellSize; }   
        else if (exitR == rows) { kRectX = exitC * cellSize; kRectY = rows * cellSize; } 
        else if (exitR == -1) { kRectX = exitC * cellSize; kRectY = -cellSize; }    
        else if (exitR >= 0 && exitR < rows && exitC >= 0 && exitC < cols) { 
