...
public class BoardPanel extends JPanel {
    ...
    private final int margin; // For drawing exits
    ...
    public BoardPanel(Board board) {
        ...
        if (board != null && board.getPieces() != null) {
        ...
        int panelWidth = (board != null ? board.getCols() : 6) * cellSize + 2 * margin;
        int panelHeight = (board != null ? board.getRows() : 6) * cellSize + 2 * margin;
        setPreferredSize(new Dimension(panelWidth, panelHeight));
        setLayout(new FlowLayout()); // Ensure it's a container
    }
    ...
    public void updateBoard(Board board) {
        this.board = board;
        // Re-assign colors if new pieces appear
        if (board != null && board.getPieces() != null) {
            for (Piece piece : board.getPieces()) {
                char id = piece.getId();
                if (id != 'P' && !pieceColors.containsKey(id)) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (board == null) return; // No board to draw

        Graphics2D g2d = (Graphics2D) g.create();
        g2d.translate(margin, margin); // Apply margin
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (board.isRowSpaceOnly(r) || board.isColSpaceOnly(c)) {
                    g2d.setColor(getBackground()); // Blank for space-only
                    g2d.fillRect(c * cellSize, r * cellSize, cellSize, cellSize);
                } else {
                    g2d.setColor(Color.LIGHT_GRAY); // Grid lines
        ...
        for (Piece piece : board.getPieces()) {
            ...
            g2d.setColor(Color.WHITE); // Piece ID text
    ...
    private void drawExit(Graphics2D g2d, int rows, int cols, FontMetrics fm) {
        ...
        if (exitR == -1 && exitC == -1 && board.getPrimaryPiece() == null) return; // No exit defined
        ...
        } else return; // Undefined K

        g2d.fillRect(kRectX + 2, kRectY + 2, kRectW - 4, kRectH - 4);
        g2d.setColor(Color.BLACK); // K text color
